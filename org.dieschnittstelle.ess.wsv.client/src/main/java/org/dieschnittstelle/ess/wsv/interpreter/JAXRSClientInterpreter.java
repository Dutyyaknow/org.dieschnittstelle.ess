    ...
    public Object invoke(Object proxy, Method meth, Object[] args)
        ...
        if (args != null && args.length > 0) {
            if (meth.getParameterAnnotations()[0].length > 0 && meth.getParameterAnnotations()[0][0].annotationType() == PathParam.class) {
                show("Annotation on first argument: " + Arrays.asList(meth.getParameterAnnotations()[0]));

                PathParam dynamicPathParam = (PathParam) meth.getParameterAnnotations()[0][0];
                show("dynamicPathParam: " + dynamicPathParam);

                String nameOfParam = dynamicPathParam.value();
                show("nameOfParam: " + nameOfParam);
                ...
                if (meth.isAnnotationPresent(Path.class)) {
                    url = StringUtils.replace(url, "{" + nameOfParam + "}", args[0].toString());
                }
                if (args.length >= 2) {
                    bodyValue = args[1];
                }
